...
class ApiKeyRow(Base):
    ...
    # Human-readable reason for the CURRENT dead/cooldown state — a short
    # provider message or probe hint ("no funds", "rate limit", the exception
    # text), not a full error log. Cleared back to NULL the moment the key is
    # confirmed alive again, so it never shows a stale reason once resolved.
