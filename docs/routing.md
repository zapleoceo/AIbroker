...
>
> **2026-09-07: groq joined `JSON_UNRELIABLE_PROVIDERS`.** The old note that groq
> stays reliable thanks to its grammar-constrained JSON mode is now the problem:
> that mode rejects our schema server-side — `400 Failed to validate JSON. Please
> adjust your prompt`, a BadRequestError, so it never showed as InvalidJSON in our
> gate. Measured over 7 days on all four groq keys: structured 553 ok / 383
> rejected (41% wasted), chat:fast 2203 ok / 258 rejected — 641 guaranteed-failed
> attempts a week. groq is sunk for JSON requests, not dropped; plain-text groq
