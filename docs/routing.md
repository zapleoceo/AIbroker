...
| `chat:smart` | **deepseek** → cerebras → groq → gemini → mistral → cohere → openrouter → sambanova → cloudflare → anthropic → openai | `llm:chat` | **The one exception to free-first (2026-07-17, owner-approved, cap $0.50→$1):** Stepan's money lane routes to ONE strong model (v4-flash, hybrid thinking) for stable quality + warm per-account prompt cache (hit input $0.0028/M ≈ $0.0004/reply). Free pool = fallback tail (deepseek flake and the budget-downgrade both walk there). Pre-positions for cerebras free-tier death 2026-08-17. |
