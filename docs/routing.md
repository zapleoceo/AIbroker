...
| `chat:smart` | **deepseek** → cerebras → groq → gemini → sambanova → cloudflare → anthropic → openai | `llm:chat` | **The one exception to free-first (2026-07-17, owner-approved, cap $0.50→$1):** Stepan's money lane routes to ONE strong model (v4-flash, hybrid thinking) for stable quality + warm per-account prompt cache (hit input $0.0028/M ≈ $0.0004/reply). **2026-07-21 quality prune (owner request "keep only providers that give good answers"):** removed **mistral** (0 successful smart calls/7d, keys AuthError), **cohere** command-r7b (25 InvalidJSON / 4 ok = 86% garbage — 7B can't do the structured reply), **openrouter** gemma-4-31b (0 ok ever / all rate-limited). Kept = clean valid answers in prod: deepseek anchor, the gpt-oss-120b trio cerebras/groq/cloudflare (0-0.4% bad), gemini-2.5-flash (0 empty), sambanova llama-3.3-70b (0 bad when it has quota), paid anthropic/openai tail. Pre-positions for cerebras free-tier death 2026-08-17. |
