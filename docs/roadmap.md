...
### Фаза 4 — общий async-слой (submit/poll для всех capability)
- Обобщить `deep_jobs` → таблица `jobs` (любая capability) + `POST /v1/jobs?capability=…` + `GET /v1/jobs/{id}`.
- sync-эндпоинты остаются, внутри зовут тот же движок.
- chat:deep = capability с `async_only=True`; спец-роут `/v1/deep` оставить как алиас (обратная совместимость).
- Клиенты (Вера, Степан) мигрируют постепенно: сначала длинные/дорогие ходы, потом остальное. **Sync не отключаем.**
- **Эффект**: клиент гарантированно получает ответ, не держа соединение; исчерпывающая ротация без упора в nginx/504.
- Тесты: submit+poll на нескольких capability; обратная совместимость `/v1/deep`.
